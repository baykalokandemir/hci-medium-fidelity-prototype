<template>
    <NuxtLink :to="computedTo" :class="[
        ...
        <div :class="[
            ...
            layout === 'horizontal' ? 'w-full sm:w-48 aspect-square rounded-lg' : 
        ...
        <div :class="[
            ...
            <!-- Horizontal Content -->
            <div v-if="layout === 'horizontal'" class="flex flex-col h-full w-full">
                <!-- Title & Ratings row -->
                ...
                <!-- Description -->
                <p class="text-sm text-slate-600 line-clamp-2 md:line-clamp-3 leading-relaxed mb-4">{{ show.description || show.subtitle }}</p>

                <!-- Bottom anchors -->
                <div class="mt-auto flex justify-between items-end">
                    <div class="flex-1 mr-4">
                        <div v-if="show.progress !== undefined" class="-mb-1">
                            <ProgressBar :percentage="show.progress" :color="progressColor" :seasons="show.seasons" />
                        </div>
                    </div>
                    <div class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-brand border border-brand/30 bg-indigo-50/50 hover:bg-indigo-50 hover:border-brand/50 rounded-lg transition-all w-max shadow-sm ml-auto">
